...
static const char *TAG = "WatcherTouch";

// State the stock driver keeps local to a single read call, which makes the
// lift indistinguishable from a "no new report" sample. Persist it and expose
// it so the board can hold a press through empty samples and only release on
// a real lift record.
static volatile bool s_touch_pressed;
static volatile bool s_points_exist;
static volatile uint16_t s_read_len;
static volatile uint8_t s_gesture;
...
esp_err_t watcher_spd2010_touch_new(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
    ...
    spd2010->get_track_id = get_track_id;
    spd2010->del = del;
    /* Mutex */
    ...
    ESP_LOGI(TAG, "Touch panel create success");
...
static esp_err_t read_data(esp_lcd_touch_handle_t tp)
{
    ...
    for (int i = 0; i < touch_cnt; i++) {
        ...
        tp->data.coords[i].track_id = touch.rpt[i].id;
    }
    portEXIT_CRITICAL(&tp->data.lock);

    /* Persist the state the stock driver keeps inside a single call: reports
     * with pressure mean the finger is down, an all-zero report is the lift.
     * Empty samples leave the state untouched so a press is never released
     * just because the controller had nothing new to say. */
    uint8_t active = 0;
    for (int i = 0; i < touch.touch_num; i++) {
        if (touch.rpt[i].weight != 0) {
            active = 1;
            break;
        }
    }
    ...
    } else if (touch.touch_num > 0) {
        s_touch_pressed = false;
    }
    s_gesture = touch.gesture;
    if (touch.touch_num > 0 || touch.gesture != 0) {
        ESP_LOGD(TAG, "evt pt=%d gest=%u len=%u cnt=%u w0=%u",
                 (int)s_points_exist, (unsigned)touch.gesture,
                 (unsigned)s_read_len, (unsigned)touch.touch_num,
                 touch.touch_num > 0 ? (unsigned)touch.rpt[0].weight : 0);
    }
...
static esp_err_t read_tp_status_length(esp_lcd_touch_handle_t tp, tp_status_t *tp_status)
{
    ...
    s_points_exist = tp_status->status_low.pt_exist != 0;
    s_read_len = tp_status->read_len;
    return ESP_OK;
}
...
static esp_err_t read_tp_hdp(esp_lcd_touch_handle_t tp, tp_status_t *tp_status, tp_touch_t *touch)
{
    ...
    /* Never trust the controller's length blindly: cap it to the buffer. */
    uint16_t read_len = tp_status->read_len;
    if (read_len > sizeof(sample_data)) {
        read_len = sizeof(sample_data);
    }
    ...
    i2c_write(&sample_data[0], 2);
    esp_rom_delay_us(200);
    i2c_read(&sample_data[0], read_len);
    esp_rom_delay_us(200);

    check_id = sample_data[4];

    /* Parse point records whenever the payload looks like one. The stock
     * driver additionally required pt_exist here, which silently dropped the
     * controller's lift record in some cases and left the finger stuck. */
    if ((check_id <= 0x0A) && (read_len >= 10)) {
        touch->touch_num = ((read_len - 4) / 6);
        ...
        if (read_len > 0) {
            ESP_LOGW(TAG, "drop report len=%u pt=%u gest=%u id=0x%02X",
                     (unsigned)read_len, (unsigned)tp_status->status_low.pt_exist,
                     (unsigned)tp_status->status_low.gesture, (unsigned)check_id);
        }
        touch->touch_num = 0x00;
        touch->gesture = 0x00;
    }
